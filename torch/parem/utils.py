...
def show_images(
    ...
    grid = (grid.detach() + 1.0) / 2  # Map from [-1, 1] to [0, 1].
...
def get_mnist(
    ...
    images = torch.stack([transform(dataset.data[i].numpy()) for i in range(n_images)])

    tensor_dataset = DatasetWithIndicesAndDetails(
        images,
...
def get_cifar10(
    ...
    images = torch.stack([transform(dataset.data[i]) for i in range(n_images)])
...
class SingleImagesFolderMTDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if cache is not None and os.path.exists(cache):
            ...
            def _map(_path_imgs):
                imgs_0 = [
                    self.transform(np.array(PIL.Image.open(os.path.join(root, p_i))))
                    for p_i in _path_imgs
                ]
...
def get_celeba(data_path: Union[str, Path], n_images: int, train: bool = True):
    """Return the CelebA dataset as `SingleImagesFolderMTDataset`"""
    dataset = SingleImagesFolderMTDataset(
        root=data_path,
        cache=None,
        num_images=n_images,
        transform=transforms.Compose(
